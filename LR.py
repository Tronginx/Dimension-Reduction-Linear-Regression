...
def linear_regression(X, Y):
    ...
    #the notation '_' means matrix multiplication
    x_xt = np.matmul(tilde_X, tilde_X.transpose())
    x_xt_inverse = np.linalg.inv(x_xt)
    x_xt_inverse_x = np.matmul(x_xt_inverse, tilde_X)
    ans = np.matmul(x_xt_inverse_x,Y)
    w = ans[:D, 0].reshape(-1, 1)
    b = ans[-1, 0]
